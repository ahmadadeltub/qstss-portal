...
const AdminPanel: React.FC = () => {
  ...
  // Function to sort students by grade, class, and name
  const getSortedStudents = () => {
    return [...students].sort((a, b) => {
      ...
      // Finally by last name, then first name
  ...
  // Group students by grade for display statistics
  ...
  const fetchStudents = async () => {
    try {
      ...
      // The apiService.getStudents() already returns the students array
      const studentsArray = Array.isArray(response) ? response : (response.data || []);
      setStudents(studentsArray);
      showNotification(`Loaded ${studentsArray.length} students`, 'success');
    } catch (error) {
  ...
  const handleUpdateTeacher = async () => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || error.response?.data?.error || 'Failed to update teacher';
  ...
  const handleCreateStudent = async () => {
    try {
      ...
      showNotification(error.response?.data?.message || 'Failed to create student', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpdateStudent = async () => {
    ...
    try {
      ...
      showNotification(error.response?.data?.message || 'Failed to update student', 'error');
    } finally {
      setLoading(false);
    }
  };
      ...
      <Paper sx={{ mb: 3 }}>
        ...
        <TabPanel value={tabValue} index={0}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
            <Typography variant="h6">Teachers Management ({teachers.length} teachers)</Typography>
            <Button
              variant="contained"
              startIcon={<AddIcon />}
              onClick={() => {
        ...
        <TabPanel value={tabValue} index={1}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
            <Box>
              <Typography variant="h6">Students Management ({students.length} students)</Typography>
              <Box sx={{ mt: 1, display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                {Object.entries(getStudentsByGrade()).map(([grade, gradeStudents]) => (
                  <Chip 
                    key={grade}
                    label={`Grade ${grade}: ${gradeStudents.length} students`}
                    size="small"
                    variant="outlined"
                    sx={{ 
                      bgcolor: 'primary.50',
